...
template < typename T >
void MergeSort<T>::sort( vector<int> &arr, unsigned begin, unsigned end )
{
    if ( begin == end )
    {  return; }
    else if ( begin + 1 == end )
...
template < typename T >
void MergeSort<T>::merge( vector<int> &arr, unsigned begin, unsigned mid, unsigned end )
{
    unsigned i = begin;
    unsigned j = mid + 1;
    vector<int> aux;
